          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">Carbon MRV System Management</p>
              </div>
            </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-green-600">+{stats.recentRegistrations} this month</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-6">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  ...
                  <p className="text-3xl font-bold text-gray-900">{stats.totalCarbon}</p>
            ...
            <CardContent className="p-6">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  ...
                  <p className="text-3xl font-bold text-gray-900">₹{(stats.totalEarnings / 100000).toFixed(1)}L</p>
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>Manage farmer registrations and profiles</CardDescription>
                  </div>
                  ...
                  {farmers.length > 0 ? (
                    farmers.slice(0, 10).map((farmer) => (
                      <div key={farmer._id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-gray-50">
                        <div className="flex items-center space-x-4">
                          <Avatar>
                            <AvatarFallback className="bg-green-100 text-green-600">
                              {farmer.name?.split(' ').map((n: string) => n[0]).join('').substring(0, 2)}
                            </AvatarFallback>
                          </Avatar>
                          <div>
                        ...
                        <div className="flex items-center space-x-4">
                          <div className="text-right">
                            <p className="text-sm font-medium">{farmer.location?.state}, {farmer.location?.district}</p>
                            <p className="text-sm text-gray-600">{farmer.farmDetails?.totalArea} ha</p>
                          </div>
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>Manage carbon offset projects and participants</CardDescription>
                  </div>
                ...
                <div className="space-y-4">
                  {projects.length > 0 ? (
                    projects.slice(0, 5).map((project) => (
                      <div key={project._id} className="border rounded-lg p-6 hover:bg-gray-50">
                        <div className="flex items-center justify-between mb-4">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Badge variant={project.status === 'implementation' ? 'default' : 'secondary'}>
                              {project.status}
                            </Badge>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                          <div>
                            ...
                            <p className="font-medium">{project.location?.state}, {project.location?.district}</p>
                          </div>
                          <div>
                            <p className="text-sm text-gray-600">Participants</p>
                            <p className="font-medium">{project.participants?.farmers?.length || 0} farmers</p>
                          </div>
                          <div>
                            <p className="text-sm text-gray-600">Carbon Target</p>
                            <p className="font-medium">{project.carbonMetrics?.projectedReductions} tCO₂e</p>
                          </div>
                          <div>
                            <p className="text-sm text-gray-600">Budget</p>
                            <p className="font-medium">₹{(project.financials?.totalBudget / 100000).toFixed(1)}L</p>
                          </div>
                        </div>
              ...
              <CardContent>
                <div className="space-y-4">
                  {measurements.map((measurement) => (
                    <div key={measurement._id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-gray-50">
                      <div className="flex items-center space-x-4">
                        <div className={`w-3 h-3 rounded-full ${
                          measurement.status === 'verified' ? 'bg-green-500' : 
                          measurement.status === 'pending' ? 'bg-amber-500' : 'bg-red-500'
                        }`}></div>
                        <div>
                          <p className="font-semibold">{measurement.type}</p>
                          <p className="text-sm text-gray-600">{measurement.farmer.name} ({measurement.farmer.farmerId})</p>
                      ...
                      <div className="flex items-center space-x-4">
                        <div className="text-right">
                          <p className="font-medium text-green-600">{measurement.carbonStored} tCO₂e</p>
                          <Badge variant={measurement.status === 'verified' ? 'default' : 'secondary'}>
                            {measurement.status}
                          </Badge>
                        </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-4">
                      ...
                      <div className="space-y-2">
                        <label className="text-sm font-medium">Carbon Price (₹ per tCO₂e)</label>

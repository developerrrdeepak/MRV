...
export interface ICarbonMeasurement extends Document {
  ...
  dataSource: 'farmer-input' | 'satellite' | 'iot-sensor' | 'field-visit' | 'drone';
  measurementType: 'biomass' | 'soil-carbon' | 'methane-emission' | 'tree-growth' | 'yield';
...
const CarbonMeasurementSchema = new Schema<ICarbonMeasurement>({
  ...
  measurementType: { 
    type: String, 
    required: true,
    enum: ['biomass', 'soil-carbon', 'methane-emission', 'tree-growth', 'yield'] 
  },
...
CarbonMeasurementSchema.index({ 'gpsLocation.latitude': 1, 'gpsLocation.longitude': 1 });

export default mongoose.model<ICarbonMeasurement>('CarbonMeasurement', CarbonMeasurementSchema);

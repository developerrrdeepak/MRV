...
export interface IFieldMeasurement extends Document {
  ...
  measurementType: 'biomass' | 'soil_carbon' | 'tree_count' | 'crop_yield' | 'water_usage' | 'satellite_data' | 'other';
  data: {
    ...
    cropStage?: 'sowing' | 'germination' | 'vegetative' | 'flowering' | 'fruiting' | 'harvest';
  ...
  methodology: {
    ...
    measurementMethod: 'manual' | 'gps' | 'drone' | 'satellite' | 'sensor' | 'app';
...
const FieldMeasurementSchema = new Schema<IFieldMeasurement>({
  ...
  measurementType: {
    type: String,
    required: true,
    enum: ['biomass', 'soil_carbon', 'tree_count', 'crop_yield', 'water_usage', 'satellite_data', 'other']
  },
  data: {
    // Biomass measurements
    ...
    cropStage: {
      type: String,
      enum: ['sowing', 'germination', 'vegetative', 'flowering', 'fruiting', 'harvest']
    },
...
FieldMeasurementSchema.virtual('ageInDays').get(function() {
  ...
  return Math.floor((now.getTime() - measured.getTime()) / (1000 * 60 * 60 * 24));
});

export const FieldMeasurement = mongoose.model<IFieldMeasurement>('FieldMeasurement', FieldMeasurementSchema);

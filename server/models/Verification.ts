...
VerificationSchema.index({
  "verificationPeriod.startDate": 1,
  "verificationPeriod.endDate": 1,
});
...
VerificationSchema.pre("save", function (next) {
  ...
  if (this.sampling.sampleSize && this.sampling.totalPopulation) {
    this.sampling.samplingRatio =
      this.sampling.sampleSize / this.sampling.totalPopulation;
  }
  // Calculate difference in carbon reductions
  if (
    this.findings.carbonReductions.claimed &&
    this.findings.carbonReductions.verified
  ) {
...
VerificationSchema.virtual("costPerCredit").get(function () {
  return this.carbonCredits.eligibleCredits > 0
    ? this.costs.totalCost / this.carbonCredits.eligibleCredits
    : 0;
});

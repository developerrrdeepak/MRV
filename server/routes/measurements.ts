...
router.post('/', authenticateToken, requireFarmer, async (req: Request, res: Response) => {
  try {
    ...
    if (measurementType === 'methane-emission' && field.cropType !== 'rice') {
      return res.status(400).json({
        success: false,
        message: 'Methane emission measurements are only applicable for rice fields'
      });
    }

    if (measurementType === 'tree-growth' && field.cropType !== 'agroforestry') {
      return res.status(400).json({
        success: false,
        message: 'Tree growth measurements are only applicable for agroforestry fields'
      });
    }
...
function calculateDataQuality(data: any): any {
  ...
  const optionalFields = ['biomassData', 'soilData', 'riceData', 'environmentalData'];
  ...
  completeness = (providedRequired / requiredFields.length) * 70 + 
...
router.get('/', authenticateToken, requireFarmer, async (req: Request, res: Response) => {
  try {
    ...
    if (startDate || endDate) {
      ...
      if (startDate) query.measurementDate.$gte = new Date(startDate as string);
...
router.get('/:measurementId', authenticateToken, requireFarmer, async (req: Request, res: Response) => {
  try {
...
// Update measurement
router.put('/:measurementId', authenticateToken, requireFarmer, async (req: Request, res: Response) => {
  try {
    const { measurementId } = req.params;
    const farmerId = req.user.farmerId;
    const {
...
// Delete measurement
router.delete('/:measurementId', authenticateToken, requireFarmer, async (req: Request, res: Response) => {
  try {
    const { measurementId } = req.params;
    const farmerId = req.user.farmerId;

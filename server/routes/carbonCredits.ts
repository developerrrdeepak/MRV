...
import { authenticateToken, requireFarmer, requireVerifier } from '../middleware/auth';
...
router.post('/calculate', authenticateToken, requireFarmer, async (req: Request, res: Response) => {
  try {
    ...
    if (!fieldId || !measurementIds || measurementIds.length === 0 || !calculationPeriod) {
      return res.status(400).json({
        success: false,
        message: 'Field ID, measurement IDs, and calculation period are required'
      });
    }
    ...
    const carbonData = await calculateCarbonSequestration(field, measurements, calculationPeriod);
    ...
    const carbonCredit = new CarbonCredit({
      ...
      verification: {
        status: 'pending',
        confidence: carbonData.calculationMethod.includes('satellite') ? 85 : 75,
        qualityScore: Math.round(measurements.reduce((sum, m) => sum + m.dataQuality.overallScore, 0) / measurements.length)
      },
      market: {
        pricePerCredit,
        totalValue,
      ...
      metadata: {
        biodiversityImpact: field.cropType === 'agroforestry' ? 'Positive - Enhanced biodiversity through tree cover' : 'Neutral',
        socialImpact: 'Positive - Additional income for smallholder farmer',
        additionalBenefits: getAdditionalBenefits(field.cropType),
        sustainabilityGoals: ['SDG 13: Climate Action', 'SDG 1: No Poverty', 'SDG 15: Life on Land']
      },
      isActive: true,
    ...
    await carbonCredit.populate([
      ...
      { path: 'measurementIds', select: 'measurementId measurementType measurementDate verificationStatus' }
    ]);
...
async function calculateCarbonSequestration(field: any, measurements: any[], period: any) {
  ...
  if (cropType === 'agroforestry') {
    ...
    if (biomassData) {
      ...
      calculationMethod = 'IPCC-2019 Agroforestry methodology with field measurements';
    }
    ...
    if (riceData) {
      ...
      calculationMethod = 'IPCC-2019 Rice cultivation methodology with SRI practices';
    }
  }
...
function calculateMethaneReduction(riceData: any, area: number): number {
  ...
  if (cultivationMethod === 'organic') reductionFactor += 0.20; // 20% reduction
...
function getAdditionalBenefits(cropType: string): string[] {
  ...
  if (cropType === 'agroforestry') {
    benefits.push('Biodiversity enhancement', 'Soil erosion control', 'Water conservation');
  } else if (cropType === 'rice') {
    benefits.push('Methane emission reduction', 'Water conservation', 'Soil health improvement');
  }
...
router.get('/', authenticateToken, requireFarmer, async (req: Request, res: Response) => {
  try {
    ...
    const credits = await CarbonCredit.find(query)
      ...
      .populate('measurementIds', 'measurementId measurementType measurementDate')
      .sort({ createdDate: -1 })
      .limit(Number(limit))
...
router.get('/:creditId', authenticateToken, requireFarmer, async (req: Request, res: Response) => {
  try {
    ...
    .populate('measurementIds', 'measurementId measurementType measurementDate dataQuality verificationStatus');
...
router.patch('/:creditId/verify', authenticateToken, requireVerifier, async (req: Request, res: Response) => {
  try {
    const { creditId } = req.params;

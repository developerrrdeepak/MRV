...
router.get('/dashboard', authenticateToken, requireFarmer, async (req: Request, res: Response) => {
  try {
    ...
    const totalCredits = carbonCredits.reduce((sum, credit) => sum + credit.creditDetails.creditsIssued, 0);
    const totalEarnings = payments
      .filter(p => p.transaction.status === 'completed')
      .reduce((sum, payment) => sum + payment.paymentDetails.amount, 0);

    // Recent activity summary
    const pendingMeasurements = recentMeasurements.filter(m => m.verificationStatus === 'pending').length;
    const verifiedCredits = carbonCredits.filter(c => c.verification.status === 'verified').length;
    const pendingPayments = payments.filter(p => p.transaction.status === 'pending').length;
...
// Get farmer's fields
router.get('/fields', authenticateToken, requireFarmer, async (req: Request, res: Response) => {
  try {
    const farmerId = req.user.farmerId;
...
router.get('/measurements', authenticateToken, requireFarmer, async (req: Request, res: Response) => {
  try {
    const farmerId = req.user.farmerId;
    const { page = 1, limit = 20, fieldId, measurementType, status } = req.query;
...
router.get('/carbon-credits', authenticateToken, requireFarmer, async (req: Request, res: Response) => {
  try {
    const farmerId = req.user.farmerId;
...
router.get('/payments', authenticateToken, requireFarmer, async (req: Request, res: Response) => {
  try {
    const farmerId = req.user.farmerId;
    const { page = 1, limit = 20, status } = req.query;
...
router.patch('/:farmerId/verification', authenticateToken, requireAdmin, async (req: Request, res: Response) => {
  try {

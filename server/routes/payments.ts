  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const totalValue = credits.reduce(
        (sum, credit) => sum + credit.market.totalValue,
        0,
      );
      const totalEarnings = credits.reduce(
        (sum, credit) => sum + credit.payment.netEarnings,
        0,
      );
      ...
      const payment = new Payment({
        ...
        metadata: {
          ...
          creditsPeriod: {
            startDate: new Date(
              Math.min(
                ...credits.map((c) => c.calculationPeriod.startDate.getTime()),
              ),
            ),
            endDate: new Date(
              Math.max(
                ...credits.map((c) => c.calculationPeriod.endDate.getTime()),
              ),
            ),
          },
        },
      ...
      await payment.populate(
        "creditIds",
        "creditId creditDetails.creditsIssued market.totalValue",
      );
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (startDate || endDate) {
        ...
        if (startDate)
          query["dates.initiatedDate"].$gte = new Date(startDate as string);
        if (endDate)
          query["dates.initiatedDate"].$lte = new Date(endDate as string);
      }

      const payments = await Payment.find(query)
        .populate(
          "creditIds",
          "creditId creditDetails.creditsIssued creditDetails.vintage",
        )
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (requiresApproval === "true")
        query["verification.requiresApproval"] = true;

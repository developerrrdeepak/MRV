  ...
  try {
    ...
    if (!name || !phone || !address || !farmDetails || !password) {
      return res.status(400).json({
        success: false,
        message:
          "Name, phone, address, farm details, and password are required",
      });
    }
    ...
    if (
      farmDetails.irrigatedArea + farmDetails.drylandArea !==
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const farmer = await Farmer.findById(req.user.farmerId).select(
        "+password",
      );

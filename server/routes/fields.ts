...
router.post('/', authenticateToken, requireFarmer, async (req: Request, res: Response) => {
  try {
    ...
    if (!fieldName || !area || !cropType || !coordinates || !centerPoint || !soilType || !irrigationType) {
      return res.status(400).json({
        success: false,
        message: 'Field name, area, crop type, coordinates, center point, soil type, and irrigation type are required'
      });
    }
    ...
    if (!coordinates.type || coordinates.type !== 'Polygon' || !coordinates.coordinates || coordinates.coordinates.length === 0) {
      return res.status(400).json({
        success: false,
...
router.get('/:fieldId', authenticateToken, requireFarmer, async (req: Request, res: Response) => {
  try {
...
router.put('/:fieldId', authenticateToken, requireFarmer, async (req: Request, res: Response) => {
  try {
    const { fieldId } = req.params;
    const farmerId = req.user.farmerId;
    const {
...
router.patch('/:fieldId/crop', authenticateToken, requireFarmer, async (req: Request, res: Response) => {
  try {
    const { fieldId } = req.params;
    const farmerId = req.user.farmerId;
    ...
    field.currentCrop = {
      ...
      expectedHarvest: currentCrop.expectedHarvest ? new Date(currentCrop.expectedHarvest) : new Date()
    };
...
// Deactivate field
router.delete('/:fieldId', authenticateToken, requireFarmer, async (req: Request, res: Response) => {
  try {
    const { fieldId } = req.params;
    const farmerId = req.user.farmerId;
    ...
    if (activeMeasurements > 0) {
      return res.status(400).json({
        success: false,
        message: 'Cannot deactivate field with pending measurements. Please complete verification first.'
      });
    }
...
router.get('/:fieldId/measurements', authenticateToken, requireFarmer, async (req: Request, res: Response) => {
  try {
    const { fieldId } = req.params;
    const farmerId = req.user.farmerId;

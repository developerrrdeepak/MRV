...
export const getSoilData: RequestHandler = async (req, res) => {
  try {
    ...
    if (!isFinite(lat) || !isFinite(lon)) {
      return res
        .status(400)
        .json({ success: false, message: "lat/lon required" });
    }
    ...
    if (isFinite(moistureRaw)) {
      ...
      else if (moistureRaw <= 10)
        moisturePercent = Math.round(moistureRaw * 10);
    ...
    res.json({
      success: true,
      data: {
        temperatureC: isFinite(temp) ? temp : null,
        moisturePercent: isFinite(moisturePercent as any)
          ? moisturePercent
          : null,

  ...
  try {
    ...
    const projects = await CarbonProject.find(filter)
      .populate(
        ...
        "name farmerId location.state location.district",
      )
      .sort({ createdDate: -1 })
      .skip(skip)
      .limit(limit)
...
export const getProjectById: RequestHandler = async (req, res) => {
  try {
    const project = await CarbonProject.findById(req.params.id)
      .populate(
        "participants.farmers",
        "name farmerId phone location farmDetails",
      )
      .lean();
    ...
    const measurementCount = await FieldMeasurement.countDocuments({
      project: project._id,
    });
...
export const createProject: RequestHandler = async (req, res) => {
  try {
    ...
    if (!req.body.projectId) {
      ...
      const typePrefix =
        req.body.projectType?.substring(0, 3).toUpperCase() || "CRB";
...
export const addFarmersToProject: RequestHandler = async (req, res) => {
  try {
    ...
    if (!Array.isArray(farmerIds) || farmerIds.length === 0) {
      return res
        .status(400)
        .json({ error: "farmerIds must be a non-empty array" });
    }
    ...
    res.json({
      ...
      timeline: {
        ...
        daysRemaining: Math.ceil(
          (new Date(project.timeline.endDate).getTime() - Date.now()) /
            (1000 * 60 * 60 * 24),
        ),
      },
      financials: {
        ...
        $facet: {
          ...
          totalProjectedReductions: [
            { $match: { isActive: true } },
            {
              $group: {
                _id: null,
                total: { $sum: "$carbonMetrics.projectedReductions" },
              },
            },
          ],
          recentProjects: [
